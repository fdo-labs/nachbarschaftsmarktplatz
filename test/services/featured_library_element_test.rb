...
class FeaturedLibraryQueryTest < ActiveSupport::TestCase
  describe '#find' do
    it 'finds featured library elements and fills them if need be' do
      ...
      value(result).must_equal library: library, exhibits: [older_featured_element.article, newer_featured_element.article]
      ...
      value(result).must_equal library: library, exhibits: [newer_featured_element.article, non_featured_element.article]
      value(non_featured_element.reload.exhibition_date).wont_be_nil
      ...
      value(result[:exhibits][0]).must_equal non_featured_element.article
      expect([older_featured_element.article, newer_featured_element.article]).must_include result[:exhibits][1] # one of the 2, it's randomized
    end
  end
end
