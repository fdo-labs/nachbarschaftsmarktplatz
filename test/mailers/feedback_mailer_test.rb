...
class FeedbackMailerTest < ActiveSupport::TestCase
  ...
  it '#feedback_and_help' do
    ...
    expect(mail).must deliver_to EMAIL_ADDRESSES['FeedbackMailer']['send_feedback']['dealer']
    expect(mail).must deliver_from EMAIL_ADDRESSES['default']
    expect(mail).must have_subject 'bazfuz'
  end
  ...
  it '#donation_partner' do
    ...
    expect(mail).must deliver_to EMAIL_ADDRESSES['FeedbackMailer']['become_donation_partner']
    expect(mail).must deliver_from EMAIL_ADDRESSES['default']
    expect(mail).must have_subject 'Spendenpartner*in Anfrage'
  end
end
